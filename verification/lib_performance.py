...
from .lib_math import probit, logit, pavx, sigmoid, optimal_llr, custom_optimal_llr
...
def rocch2eer(pmiss, pfa):
    ...
    for i in range(pfa.shape[0] - 1):
        ...
        assert (xx[1] <= xx[0]) & (yy[0] <= yy[1]), "pmiss and pfa have to be sorted"
...
def min_cllr(tar_llrs, nontar_llrs, monotonicity_epsilon=1e-6):
    [tar, non] = optimal_llr(
        tar_llrs, nontar_llrs, laplace=False, monotonicity_epsilon=monotonicity_epsilon
    )
    cmin = cllr(tar, non)
    return cmin


def custom_min_cllr(scores, labels, Ntar, Nnon, monotonicity_epsilon=1e-6):
    [tar, non] = custom_optimal_llr(
        scores,
        labels,
        Ntar,
        Nnon,
