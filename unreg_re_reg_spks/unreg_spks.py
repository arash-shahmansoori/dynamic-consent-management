...
from .train_unreg_epoch import train_unreg_per_epoch
...
def unreg_sup(
    ...
    paths_filenames = create_filenames_unreg_results(
        args,
        hparams,
        args.spk_per_bucket,
        unreg_spks,
    ...
    if ckpt_cls is not None:
        ...
        if start_epoch_available and status_cls == "unreg_cls":
    ...
    early_stopping_status = {
        bkt: False for bkt in buckets if bkt not in unreg_bkts_storage
    }
    ...
    for epoch in range(start_epoch, start_epoch + args.epoch):
        ...
        # Break the training loop if all the buckets with unreg speakers
        # have ``True'' early stopping status and the early stopping status of the
        # rest of the buckets after completion of progressive registrations is ``True''
