...
class TestCreateMultiStridedSamples:
    """
    Create multi-strided samples for the training.
    """

    def __init__(self, max_mem, dim_emb, n_utterances_labeled):

        self.max_mem = max_mem
        self.dim_emb = dim_emb
        self.n_utterances_labeled = n_utterances_labeled
    ...
    def num_per_spk_utts_progressive_mem(
        ...
        utts_per_spk = torch.floor(
            torch.tensor((self.max_mem / (total_spks_per_bkts_storage)))
        )
    ...
    #     utts_per_spk = torch.floor(
    #         torch.tensor(
    #             (self.max_mem / (num_buckets * (spk_per_bucket + spk_per_bucket_reg)))
    #         )
    #     )
    #     return int(utts_per_spk)
        ...
        l = [
            (
                torch.from_numpy(
                    prng.choice(
                        range(
                            self.n_utterances_labeled * i,
                            self.n_utterances_labeled * (i + 1),
                        ),
                        num_utts,
                        replace=True,
                    )
    ...
    def inter_bucket_sample(
        ...
        feats_init.append(
            per_bkt_samples[per_bkt_indices_selected, :].view(-1, self.dim_emb)
        )
        labels_init.append(per_bkt_labels[per_bkt_indices_selected].view(-1))

        stacked_feats = torch.cat(feats_init, dim=0).view(-1, self.dim_emb)
